...
runtime for the `retry` feature in the JavaScript target — do
...
| Runtime template | `.sdk/tm/js/src/feature/retry/` (copied here on `generate`; `FEATURE_Name`/`FEATURE_VERSION` substituted) |
...
- **Behaviour**: edit the runtime template under
  `.sdk/tm/js/src/feature/retry/`, then regenerate.
