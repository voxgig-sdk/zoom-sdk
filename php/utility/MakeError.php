...
class ZoomMakeError
{
    public static function call(?ZoomContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ZoomContext([], null);
        }
        $op = $ctx->op ?? new ZoomOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ZoomResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ZoomError) ? $err->msg : (string)$err;
