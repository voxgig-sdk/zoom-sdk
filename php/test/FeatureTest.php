...
class FtCtrl extends ZoomControl
{
...
class FtClient
{
    ...
    private ZoomUtility $utility;

    public function __construct(ZoomUtility $utility, array $options)
    {
    ...
    public function get_utility(): ZoomUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ZoomContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ZoomContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ZoomConfig::make_feature($name);
    ...
    private static function buildUrl(ZoomSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ZoomError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ZoomResponse($fetched);
        }
    ...
    private function fail(ZoomContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ZoomContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
