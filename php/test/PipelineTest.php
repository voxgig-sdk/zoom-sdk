...
{
    private static function utility(): ZoomUtility
    {
        return new ZoomUtility();
    }
    ...
    private static function ctx(array $over = []): ZoomContext
    {
        ...
        $ctx->op = new ZoomOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ZoomError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ZoomOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ZoomMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ZoomOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ZoomMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ZoomOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ZoomMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ZoomSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ZoomMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx([
            'spec' => new ZoomSpec(['step' => 's']),
            'response' => new ZoomResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ZoomResult([]),
        ]);
        [, $err] = ZoomMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ZoomSpec(['step' => 's']),
            'response' => new ZoomResponse(self::resp(200, ['v' => 1])),
            'result' => new ZoomResult([]),
        ]);
        [, $err] = ZoomMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ZoomSpec(['step' => 's']),
            'response' => new ZoomResponse(self::resp(200, ['v' => 2])),
            'result' => new ZoomResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ZoomMakeResponse::call($ctx);
    ...
    {
        $preset = new ZoomResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ZoomSpec([]),
            'response' => new ZoomResponse([]),
            'result' => new ZoomResult([]),
        ...
        [$got, $err] = ZoomMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ZoomResult([])]);
        [, $err] = ZoomMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ZoomSpec([]), 'result' => null]);
        [, $err] = ZoomMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ZoomSpec(['step' => 's']),
            'result' => new ZoomResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ZoomOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ZoomMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ZoomSpec(['step' => 's']),
            'result' => new ZoomResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ZoomOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ZoomMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ZoomResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ZoomSpec([]),
            'result' => new ZoomResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ZoomMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new ZoomError('boom', 'boom');
        $utility->fetcher = function (ZoomContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ZoomSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ZoomMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ZoomContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ZoomMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ZoomResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ZoomContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ZoomSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ZoomMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ZoomResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ZoomSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ZoomMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ZoomMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ZoomResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ZoomDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ZoomResult(['ok' => false])]);
        $this->expectException(ZoomError::class);
        ZoomDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ZoomResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ZoomMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ZoomBaseFeature();
        $b = new ZoomBaseFeature();
        ZoomFeatureAdd::call($ctx, $a);
        ZoomFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ZoomBaseFeature
    {
        $f = new ZoomBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): ZoomContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ZoomSpec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new ZoomResponse(['headers' => null]),
            'result' => new ZoomResult([]),
        ]);
        ZoomResultHeaders::call($ctx);
