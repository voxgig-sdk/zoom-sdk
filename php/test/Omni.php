...
//    only - a typed ZoomContext there would make every ctx
...
final class ZoomOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new ZoomContext($clean, null);
    ...
    public static function err_from_map($m): ?ZoomError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new ZoomError(is_string($code) ? $code : '', $msg);
    }
