...
[MCP](https://modelcontextprotocol.io) server exposing the Zoom SDK as
two agent tools — `zoom_list` and `zoom_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/zoom-mcp)
make build
...
claude mcp add --scope user zoom \
  -- /absolute/path/to/zoom-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./zoom-mcp -transport http -addr :8080
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/zoom-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user zoom \
     -- "$PWD"/dist/*/zoom-mcp -transport stdio
   ```
...
| Tool | Args | Returns |
|------|------|---------|
| `zoom_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `zoom_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./zoom-mcp -transport http -addr :18080 &
