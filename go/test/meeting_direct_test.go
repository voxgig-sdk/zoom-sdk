...
func TestMeetingDirect(t *testing.T) {
	...
	t.Run("direct-load-meeting", func(t *testing.T) {
		setup := meetingDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-meeting", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		if setup.live {
			...
			listParams["user_id"] = setup.idmap["user01"]
...
func meetingDirectSetup(mockres any) *meetingDirectSetupResult {
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["ZOOM_TEST_MEETING_ENTID"]; ok {
