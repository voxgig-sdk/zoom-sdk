...
import (
	...
	omni "github.com/voxgig-sdk/zoom-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ZoomError); is {
				out := map[string]any{
					"name":    "ZoomError",
...
func omniCtx(arg any, client *sdk.ZoomSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ZoomSDK); is {
			client = live
