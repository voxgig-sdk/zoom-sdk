...
const TEST_SEED = {"entity":{"meeting":{"test01":{"id":"test01"}}}}
...
function isRunnable(code: string): boolean {
  return (
    /new\s+ZoomSDK\b/.test(code) ||
    /\bZoomSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ZoomSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ZoomSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
